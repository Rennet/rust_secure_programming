...
use google_authenticator::{verify_code, ErrorCorrectionLevel, GoogleAuthenticator, GA_AUTH};
use hex;
use hex::FromHex;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match read_credential(USERNAME_DB) {
        Ok(credential) => {
            ...
            let password = sanitize_password(SecretBox::new(Box::new(
        ...
        Err(_) => {
            ...
            let password_again = sanitize_password(SecretBox::new(Box::new(
                read_password().expect("Failed to read password"),
            )))?;
    ...
    loop {
        ...
        match commands[selection] {
            ...
            "Store" => {
                println!("Do you want to list files in the current directory? (yes/no):");
                let commands = vec!["Yes", "No"];
                // Display the menu and let the user select a command
                let selection = Select::with_theme(&ColorfulTheme::default())
                    .with_prompt("Choose a command")
                    .items(&commands)
            ...
            "Retrieve" => {
                ...
                let retrieve = file_retrieve(full_file.into(), key);
                match retrieve {
                    Ok(_) => {
                    ...
                    Err(error) => {
                        let log_message = format!("Authenticated user failed to retrieve file. Due to: {error}");
                        log_to_event_viewer(&log_message, EVENTLOG_INFORMATION_TYPE);
                    }
                }
            }
            ...
            "Encrypt Text" => {
                ...
                println!(
                    "Ciphertext: {}",
                    encryption::text_encryption(plaintext, key)
                );
            }
            "Decrypt Text" => {
                ...
                println!(
                    "Plaintext: {}",
                    encryption::text_decryption(ciphertext, key)
                );
            }
            "Quit" => {
...
fn register(
    ...
    password_again: SecretBox<String>,
    auth: GoogleAuthenticator,
    config: Config<'_>,
) -> Result<(), Box<dyn std::error::Error>> {
    if username.len() < 5 {
        return Err("Username is too short, please select a longer one."
            .to_string()
            .into());
    }
    ...
    if password.expose_secret().trim() == password_again.expose_secret().trim() {
        ...
        println!(
            "Your username is: {} and your password has been set.",
        ...
        let (salt, derived_key) =
            derive_key_from_password(password, None).expect("Key derivation failed");
        let salt_h = SecretBox::new(Box::new(hex::encode(&salt)));

        let hash_password = argon2::hash_encoded(
            account_password.clone().unwrap().as_bytes(),
            salt.as_slice(),
            &config,
        )
        .unwrap();
        let hash_username = argon2::hash_encoded(
            account_name.clone().unwrap().as_bytes(),
            salt.as_slice(),
            &config,
        )
        .unwrap();
        let hash_salt_name =
            argon2::hash_encoded(salt_name.clone().as_bytes(), salt.as_slice(), &config).unwrap();
        //set MFA

        let _ = write_credential_custom(
            USERNAME_DB,
            Credential {
        ...
        // Convert it to MFA
        let base32_secret =
            SecretBox::new(Box::new(encode(Rfc4648 { padding: true }, &derived_key)));
        ...
        if argon2::verify_encoded(&credential.secret, password.expose_secret().as_bytes()).unwrap()
        {
            ...
            match read_credential(SALT_DB) {
                Ok(credential) => {
                    ...
                    let Ok((_salt, derived_key)) =
                        derive_key_from_password(password, decoded_secret.as_deref())
                    else {
                        todo!()
                    };
                    let base32_secret =
                    ...
                    if verify_code!(&base32_secret.expose_secret(), &mfa_code, 1, 0) {
                        ...
                        let log_message =
                            format!("{username} Failed to authenticate - incorrect MFA.");
                        log_to_event_viewer(&log_message, EVENTLOG_INFORMATION_TYPE);
                        quit();
                    }
                }
...
fn file_deletion(file_path: PathBuf) -> io::Result<()> {
    ...
    if !path.exists() {
        let log_message = format!("Authenticated user tried to delete {}.", &path.display());
        log_to_event_viewer(&log_message, EVENTLOG_INFORMATION_TYPE);
...
pub fn write_credential_custom(
    target: &str,
    val: Credential,
) -> Result<(), Box<dyn std::error::Error>> {
    //Alteration due to persistence
...
pub fn sanitize_password(
    password: SecretBox<String>,
) -> Result<SecretBox<String>, Box<dyn std::error::Error>> {
    ...
    if sanitized.expose_secret().len() < 8 {
        return Err("Password has to be at least 8 characters"
            .to_string()
            .into());
    }

    if sanitized.expose_secret().chars().any(|c| c.is_control()) {
        return Err("Password contains invalid control characters."
            .to_string()
            .into());
    }
...
mod tests {
    ...
    fn register_test(
        ...
        password: SecretBox<String>,
        password_again: SecretBox<String>,
        base32_secret: SecretBox<String>,
        auth: GoogleAuthenticator,
        username_db: &str,
        config: Config<'_>,
    ) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if password.expose_secret().trim() == password_again.expose_secret().trim() {
            ...
            log_to_event_viewer(&log_message, EVENTLOG_INFORMATION_TYPE);
            let hash_password =
                argon2::hash_encoded(account_password.clone().unwrap().as_bytes(), salt, &config)
                    .unwrap();
            let hash_username =
                argon2::hash_encoded(account_name.clone().unwrap().as_bytes(), salt, &config)
                    .unwrap();
    ...
    fn login_test(
        ...
        // INCLUDES MFA CODE, REAL FUNCTION PROMPTS IT
        // Verifying credentials
        if argon2::verify_encoded(&credential.username, username.as_bytes()).unwrap() {
            if argon2::verify_encoded(&credential.secret, password.expose_secret().as_bytes())
                .unwrap()
            {
                let mfa_code = code.trim();
    ...
    fn test_valid_aes_key() {
        ...
        assert!(
            result.is_some(),
            "Valid key should return Some with the GenericArray"
        );
        assert_eq!(
            result.unwrap().len(),
            32,
            "The length of the AES key should be 32 bytes"
        );
    }
    ...
    fn test_invalid_length_key_too_short() {
        ...
        assert!(
            result.is_none(),
            "Key with less than 64 characters should return None"
        );
    }
    ...
    fn test_invalid_length_key_too_long() {
        ...
        assert!(
            result.is_none(),
            "Key with more than 64 characters should return None"
        );
    }
    ...
    fn test_invalid_hex_key() {
        ...
        assert!(
            result.is_none(),
            "Key with invalid hexadecimal characters should return None"
        );
    }
    ...
    fn test_edge_case_one_character_short() {
        ...
        assert!(
            result.is_none(),
            "Key with 63 characters should return None"
        );
    }
    ...
    fn test_edge_case_one_character_long() {
        ...
        assert!(
            result.is_none(),
            "Key with 65 characters should return None"
        );
    }
    ...
    fn test_file_deletion_success() {
        ...
        let file_path =
            create_local_file_with_content("test_deletion_success.txt", file_content).unwrap();
        ...
        assert!(
            !file_path.exists(),
            "File should be deleted after file_deletion"
        );
    }
    ...
    fn test_file_deletion_empty_file() {
        ...
        assert!(
            result.is_ok(),
            "file_deletion should succeed on an empty file"
        );
        assert!(
            !file_path.exists(),
            "File should be deleted after file_deletion"
    ...
    fn test_file_deletion_with_random_data() {
        ...
        let file_path =
            create_local_file_with_content("test_random_data.txt", &file_content).unwrap();
    ...
    fn test_file_deletion_after_multiple_overwrites() {
        ...
        thread::sleep(Duration::new(1, 0));
        let file_path =
            create_local_file_with_content("test_overwrites.txt", file_content).unwrap();
        ...
        assert!(
            result.is_ok(),
            "file_deletion should succeed after overwriting"
        );
        assert!(
            !file_path.exists(),
            "File should be deleted after file_deletion"
    ...
    fn cleanup_test_files() {
        ...
        remove_dir_all(TEST_FILES_DIR)
            .unwrap_or_else(|_| println!("Failed to clean up test files."));
    }
    ...
    fn test_login_success() {
        ...
        let credential = Credential {
            username: argon2::hash_encoded(
                username.as_bytes(),
                b"salt12345678",
                &Config::default(),
            )
            .unwrap(),
            secret: argon2::hash_encoded(
                password.expose_secret().as_bytes(),
                b"salt12345678",
                &Config::default(),
            )
    ...
    fn test_register_password_mismatch2() {
        ...
        assert!(
            result.is_err(),
            "Registration succeeded despite password mismatch."
        );
    }
    ...
    fn test_register_empty_username() {
        ...
        assert!(
            result.is_err(),
            "Registration succeeded with empty username."
        );
    }
    ...
    fn test_sanitize_file_input_invalid_permissions() {
        // Create a test file
        let test_file =
            create_local_file_with_content("test_file_no_read.txt", b"test content").unwrap();
        ...
        assert!(
            matches!(result, Err(FileError::InvalidPermissions)),
            "Expected InvalidPermissions, got: {:?}",
            result
        );
